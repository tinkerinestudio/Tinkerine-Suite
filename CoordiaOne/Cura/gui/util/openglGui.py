...
class glGuiPanel(glcanvas.GLCanvas):
	...
	def _drawGui(self):
		if self._glButtonsTexture is None:
			...
			self._glButtonsTexture = opengl.loadGLTexture('glButtons copy settings3.png')
			...
			self._glSettingBorder = opengl.loadGLTexture('setting_border4.png')
			self._glSettingBorder2 = opengl.loadGLTexture('setting_border5.png')
		...
		#testTooltip = tooltipText.tooltipText(20, 130, "CAMERA VIEW", "font.ttf", 24, (139,197,63)) #(43,182,115)
		glBindTexture(GL_TEXTURE_2D, self._glCoordiaTexture)

		glEnable(GL_TEXTURE_2D)
		glPushMatrix()
		glColor4f(1,1,1,1)
		glTranslate(512, 64,0)
		glScale( 512,64,0)
		...
		if self.settingsOpen:
			...
			glColor4f(1,1,1,1)
			if self.advancedOpen:
				glTranslate(380+12, size.GetHeight()-15,0)
				glScale(512,256,100)
		...
		if self.cameraOpen:
			...
			glTranslate(x, size.GetHeight()-75,0)
			glScale(512,128,100)
			glBegin(GL_QUADS)
			glTexCoord2f(1, 0)
			glVertex2f(0,-1)
		...
		if self.viewmodeOpen:
			...
			#self._container._parent.xText.setTooltip(str(x))
			if str(x)[-1] == "0":
				x += 0.5
			#if x < 624:
			#	x = 624
		...
		if self.saveMenuOpen:
			glBindTexture(GL_TEXTURE_2D, self._glSDTexture)
			glEnable(GL_TEXTURE_2D)
			glPushMatrix()
			glColor4f(1,1,1,1)
		...
		#	self.abc = testtext.font_data("font.ttf",12)
		#	self.loaded = True
		...
		#argh = tooltipText.tooltipText(200, size.GetHeight()-120, "PROFILE", "font.ttf", 16, (255,111,22))
...
class glGuiLayoutButtons(object):
	...
	def update(self):
		...
		for ctrl in self._parent._glGuiControlList:
			...
			try:
				if ctrl._center == "left" or ctrl._center == "top":
...
class glButton(glGuiControl):
	...
	def draw(self):
		...
		if self._focus:
			...
			glDisable(GL_TEXTURE_2D)
			glPopMatrix()			
			if 1: #removed image for hover over
				#blag = opengl.loadGLTexture2(self._tooltipTextTitle.makeText())
				#glBindTexture(GL_TEXTURE_2D, blag)
				#testTooltip = tooltipText.tooltipText(20, 130, "CAMERA VIEW", "font.ttf", 24, (139,197,63)) #(43,182,115)
				...
				if self._tooltipTitle != '':
					glBindTexture(GL_TEXTURE_2D, opengl.loadGLTexture2(self._tooltipTextTitle.makeText(), self._tooltipTextTitle._textWidth, self._tooltipTextTitle._textHeight))
					self._tooltipTextTitle.displayText2()
				if self._tooltip != '':
					glBindTexture(GL_TEXTURE_2D, opengl.loadGLTexture2(self._tooltipText.makeText(), self._tooltipText._textWidth, self._tooltipText._textHeight))
...
class glLabel(glGuiControl):
	def __init__(self, parent, label, pos, size = 11, colour = (0,0,0,255)):
		...
		self._center = None
		self._tooltipText = tooltipText.tooltipText(200, 200, self._label, "font.ttf", size, colour) #(139,197,63)) #(43,182,115)
		self._tooltipText.makeText()
		self._hidden = False
...
class glNumberCtrl(glGuiControl):
	def __init__(self, parent, value, pos, callback, isPercent = False):
		...
		self._tooltipText = tooltipText.tooltipText(200, 200, self._value, "font.ttf", 13, (43,182,115)) #(139,197,63)) #(43,182,115)
		self._tooltipText.makeText()
	...
	def draw(self):
		...
		glColor4ub(0,0,0,255)
		if self.hasFocus():
			glTranslate(x+((opengl.glGetStringSize(self._value[0:self._selectPos])[0] - 1)*0.8), y-6, 0)
...
class glTextLabelSetting(glGuiControl):
	def __init__(self, parent, tooltip, pos, Xnudge = 0, Ynudge = 0, fontSize = 12, advanced = False, colour = (50,50,50)):
		...
		self._tooltipText = tooltipText.tooltipText(200, 200, self._tooltip, "font.ttf", self._fontSize, self._colour) #(43,182,115)
		#self._tooltipText.makeText()
