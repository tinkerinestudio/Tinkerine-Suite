...
class SpeedRepository(object):
	...
	def __init__(self):
		...
		self.topLayerFlowRateMultiplier = settings.FloatSpin().getFromValue(0.0, 'Top layer flow rate (ratio):', self, 10.0, 1.0)
		########
		#try:
		#	#abc = settings.getReadRepository(carve.getNewRepository())
		#	print skeinforge_polyfile.getFileOrDirectoryTypesUnmodifiedGcode(self.fileNameInput.value, fabmetheus_interpret.getImportPluginFileNames(), self.fileNameInput.wasCancelled)
...
class SpeedSkein(object):
	...
	def addFlowRateLine(self):
		...
		if self.isBridgeLayer and not self.isEdgePath:
			flowRate *= self.repository.bridgeFlowRateMultiplier.value
			print "a bridge layer!"
		if self.isEdgePath:
		...
		if self.isTopLayer:
			flowRate *= self.repository.topLayerFlowRateMultiplier.value*5
			
		########
		#if self.layerIndex >49-3-2 and not self.isEdgePath:
		#	flowRate = self.repository.flowRateSetting.value * self.repository.bottomLayerFlowRateMultiplier.value
