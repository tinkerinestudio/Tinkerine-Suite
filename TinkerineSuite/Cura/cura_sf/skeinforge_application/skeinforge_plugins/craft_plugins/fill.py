...
class FillRepository(object):
	...
	def __init__(self):
		...
		self.diaphragmPeriod = settings.IntSpin().getFromValue( 20, 'Diaphragm Period (layers):', self, 200, 0 )
		...
		self.bottomSurfaceThickness = settings.IntSpin().getFromValue(0, 'Bottom Surface Thickness (layers):', self, 5, 2)
		self.topSurfaceThickness = settings.IntSpin().getFromValue(0, 'Top Surface Thickness (layers):', self, 5, 3)
...
#			return
		...
		#self.distanceFeedRate.addLine('(<abc>)')
		bottomThickness = self.repository.bottomSurfaceThickness.value - self.solidSurfaceThickness
		topThickness = self.repository.topSurfaceThickness.value - self.solidSurfaceThickness +1
		#print topThickness
		#print topThickness
		
		
		#bottomThickness = topSurfaceThickness - solidSurfaceThickness
		if layerRemainder >= int(round(self.repository.diaphragmThickness.value)):#j
			
			for surroundingIndex in xrange(1+bottomThickness, self.solidSurfaceThickness +1+bottomThickness):
		...
		#else:	
			#self.distanceFeedRate.addLine('(<abc> %s )' % layerRotation)
...
#		aroundWidth = 0.34321 * self.infillWidth
	...
	def addRotatedCarve(self, currentLayer, layerDelta, reverseRotation, surroundingCarves):
		...
		if layerIndex >= len(self.rotatedLayers):
			print "top"
			return "layerIndex"
		#self.distanceFeedRate.addLine('(<abc> %s )' % rotatedLayer.z)
		layerDifference = abs(layerDelta)
